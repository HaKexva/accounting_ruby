...
| `/budgets` | Budgets — two-part layout (summary + entry) like actual expenditure; **budget type** dropdown in the **entry card header** (right of the title) switches income vs expenditure |
...
|------|------|
| `/` | 儀表板 — **實際支出** |
| `/budgets` | 預算 — 與實際支出相同**上下兩區**（本月資料／登錄）；**預算類型**下拉選單在**登錄卡片標題列右側**，切換收入／支出 |
